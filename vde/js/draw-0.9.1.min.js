...
if(mode=="moving1")
{p.setNewline(mouse[0],mouse[1],p.mouseX-cardinal[0]-center[0],p.mouseY-cardinal[1]-center[1]);p.changeSelected(target).moveP1([newline[2],newline[3]]);}
if(mode=="moving2")
{var moved=[(p.mouseX-cardinal[0]-center[0])-mouse[0],(p.mouseY-cardinal[1]-center[1])-mouse[1]];p.setNewline(oldline[0]+moved[0],oldline[1]+moved[1],oldline[2]+moved[0],oldline[3]+moved[1]);p.changeSelected(target).moveP1([newline[2],newline[3]]);p.changeSelected(target).moveP2([newline[0],newline[1]]);}
if(mode=="moving3")
{p.setNewline(p.mouseX-cardinal[0]-center[0],p.mouseY-cardinal[1]-center[1],mouse[0],mouse[1]);p.changeSelected(target).moveP2([newline[0],newline[1]]);}};p.mouseMoved=function()
{var el;if(mode!="drag")
...
{target=value;if(target<0)target=0;p.setSelected(target);p.wait();$("#canvas")[0].focus();};p.setPretarget=function(value)
...
{$("#selectedZ span").text("-");$("#selectedX1").val("");$("#selectedY1").val("");$("#selectedX2").val("");$("#selectedY2").val("");$("#selectedColor").val("");$("#selectedWeight").val("");$("#selectedAlpha").val("");}else{var el=els[target];$("#selectedZ span").text(target);$("#selectedX1").val(el.x1);$("#selectedY1").val(el.y1);$("#selectedX2").val(el.x2);$("#selectedY2").val(el.y2);$("#selectedColor").val(el.color);$("#selectedWeight").val(el.weight);$("#selectedAlpha").val(el.alpha);}}
p.changeSelected=function(tar)
{return{moveP1:function(value){$(selectedX1).val(value[0]);$(selectedY1).val(value[1]);},moveP2:function(value){$(selectedX2).val(value[0]);$(selectedY2).val(value[1]);},x1:function(value){$(selectedX1).val(value)},y1:function(value){$(selectedY1).val(value)},x2:function(value){$(selectedX2).val(value)},y2:function(value){$(selectedY2).val(value)}};}
p.setColor=function(value){color=value;};p.setWeight=function(value){weight=value;};p.setAlpha=function(value){alpha=value/100;};p.setForeground=function(value){foreground^=1;return foreground;};p.getColor=function(){return color;};p.getWeight=function(){return weight;};p.getAlpha=function(){return alpha*100;};p.getForeground=function(){return foreground;};p.toggleTree=function(tar){layers[tar].toggleTree();p.wait();};p.toggleDisplay=function(tar){layers[tar].toggleDisplay();p.wait();};p.getLayer=function(){return layer;};p.getLayers=function(){return layers;};p.BACKGROUND=function(c)
...
p.addLayer=function()
{els=layers[layer.addLayer()].elements;p.setTarget(-1);p.wait();}
p.delLayer=function(tar)
{els=layers[layer.delLayer(tar)].elements;p.setTarget(-1);p.wait();}
p.updateLine=function(tar)
{var res=layers[layer.target].updateLine(tar);p.setSelected();return res;}
p.elementOrderUp=function()
{p.setTarget(layers[layer.target].orderUp(target));p.wait();}
p.elementOrderDown=function()
{p.setTarget(layers[layer.target].orderDown(target));p.wait();}
p.layerOrderDown=function(tar)
{layer.orderDown(tar);p.setTarget(-1);p.wait();}
p.layerOrderUp=function(tar)
{layer.orderUp(tar);p.setTarget(-1);p.wait();}
p.layerSetLink=function(tar,linkGround)
{layers[tar].setLink(linkGround);p.wait();}
...
{layers[layer.target].addLine([xy1[0],xy1[1]],[xy2[0],xy2[1]],color,weight,alpha,foreground);p.updateEls();p.setTarget(els.length-1);}
