...
{var this_=this;return{xy1:function(xy){xy[0]=Math.round(xy[0]);xy[1]=Math.round(xy[1]);this_.elements[target].x1=xy[0];this_.elements[target].y1=xy[1];},xy2:function(xy){xy[0]=Math.round(xy[0]);xy[1]=Math.round(xy[1]);this_.elements[target].x2=xy[0];this_.elements[target].y2=xy[1];},color:function(color_){this_.elements[target].color=color_;},weight:function(weight_){this_.elements[target].weight=weight_;},alpha:function(alpha_){this_.elements[target].alpha=alpha_;},foreground:function(foreground_){this_.elements[target].color=foreground_;},display:function(display_){if(display_==undefined)this_.elements[target].display^=1;else this_.elements[target].display=display_;}};},updateCurve:function()
...
{this.x1=xy1[0];this.y1=xy1[1];this.x2=xy2[0];this.y2=xy2[1];this.cx=c[0];this.cy=c[1];this.color=color;this.weight=weight;this.alpha=alpha;this.foreground=foreground;this.display=1;}};var Draw=function(p){var winX=window.innerWidth;var winY=window.innerHeight;var cardinal=[winX/2-400,winY/2-300];var center=[0,0];var mode="new";var mouse=[0,0];var newline=[0,0,0,0];var newlines=[];var oldline=[0,0,0,0];var target=-1;var pretarget=-1;var wait=0;var sorted=0;var ctx=canvas.getContext('2d');var img=new Image();p.imgAlpha=0.5;p.imgX=0;p.imgY=0;var backgroundColor="#6a7495";var weight=2;var color="ffffff";var alpha=1;var foreground=0;var helper={start:1,end:1,middle:1,target:1};var layer=new Layers();var layers=layer.layers;var els=layers[0].elements;p.setup=function()
...
{if(p.mouseButton!=p.LEFT)return;wait=-1;mouse=[p.mouseX-cardinal[0]-center[0],p.mouseY-cardinal[1]-center[1]];if(mode=="new")newline=[mouse[0],mouse[1],mouse[0],mouse[1]];if(mode=="move1"||mode=="move2"||mode=="move3")target=pretarget;};p.mouseReleased=function()
{if(p.mouseButton!=p.LEFT)return;if(p.mouseButton!=p.LEFT)return;wait=-1;if(mode=="new")
{if(newline[0]!=newline[3]&&newline[2]!=newline[4])
{layers[layer.target].addLine([newline[0],newline[1]],[newline[2],newline[3]],color,weight,alpha,foreground);p.updateEls();target=els.length-1;}
newline=[0,0,0,0];}
if(mode=="moving1"||mode=="moving3")
{p.cursor(p.ARROW);mode="new";layers[layer.target].updateLine(pretarget).xy1([newline[2],newline[3]]);layers[layer.target].updateLine(pretarget).xy2([newline[0],newline[1]]);layers[layer.target].updateLine(pretarget).display(1);newline=[0,0,0,0];}
if(mode=="moving2")
{layers[layer.target].updateLine(pretarget).xy1([newline[2],newline[3]]);layers[layer.target].updateLine(pretarget).xy2([newline[0],newline[1]]);layers[layer.target].updateLine(pretarget).display(1);newline=[0,0,0,0];}};p.mouseDragged=function()
{if(p.mouseButton!=p.LEFT)return;wait=-1;if(mode=="drag")center=[center[0]+(p.mouseX-cardinal[0]-center[0])-mouse[0],center[1]+(p.mouseY-cardinal[1]-center[1])-mouse[1]];if(mode=="new")newline=[mouse[0],mouse[1],p.mouseX-cardinal[0]-center[0],p.mouseY-cardinal[1]-center[1]];if(mode=="move1")
{mode="moving1";mouse=[els[pretarget].x2,els[pretarget].y2];newline=[mouse[0],mouse[1],mouse[0],mouse[1]];target=pretarget;layers[layer.target].updateLine(pretarget).display(0);}
if(mode=="move3")
{mode="moving3";mouse=[els[pretarget].x1,els[pretarget].y1];newline=[mouse[0],mouse[1],mouse[0],mouse[1]];target=pretarget;layers[layer.target].updateLine(pretarget).display(0);}
if(mode=="move2")
{mode="moving2";mouse=[(els[pretarget].x1+els[pretarget].x2)/2,(els[pretarget].y1+els[pretarget].y2)/2];oldline=[els[pretarget].x2,els[pretarget].y2,els[pretarget].x1,els[pretarget].y1];target=pretarget;layers[layer.target].updateLine(pretarget).display(0);}
if(mode=="moving1")
{newline=[mouse[0],mouse[1],p.mouseX-cardinal[0]-center[0],p.mouseY-cardinal[1]-center[1]];}
if(mode=="moving2")
{var moved=[(p.mouseX-cardinal[0]-center[0])-mouse[0],(p.mouseY-cardinal[1]-center[1])-mouse[1]];newline=[oldline[0]+moved[0],oldline[1]+moved[1],oldline[2]+moved[0],oldline[3]+moved[1]]}
if(mode=="moving3")
{newline=[p.mouseX-cardinal[0]-center[0],p.mouseY-cardinal[1]-center[1],mouse[0],mouse[1]];}};p.mouseMoved=function()
{var el;if(mode!="drag")
...
{if(p.keyCode==27||p.keyCode==82)center=[0,0];if(p.keyCode==32)mode="drag";var el=els[target];if(p.keyCode==37&&target!=-1)layers[layer.target].moveElement(target,[-1,0],[-1,0]);if(p.keyCode==38&&target!=-1)layers[layer.target].moveElement(target,[0,-1],[0,-1]);if(p.keyCode==39&&target!=-1)layers[layer.target].moveElement(target,[1,0],[1,0]);if(p.keyCode==40&&target!=-1)layers[layer.target].moveElement(target,[0,1],[0,1]);if(p.keyCode==83)setHelper("start");if(p.keyCode==77)setHelper("middle");if(p.keyCode==69)setHelper("end");if(p.keyCode==84)setHelper("target");p.wait();};p.keyReleased=function()
{wait=-1;if(p.keyCode==32)mode="new";if(p.keyCode==127||p.keyCode==68)
{layers[layer.target].delElement(target);setTarget(target-1);}};p.resize=function()
{wait=-1;winX=window.innerWidth;winY=window.innerHeight;cardinal=[winX/2-400,winY/2-300];p.size(winX,winY);p.redraw();};p.wait=function()
...
{target=value;if(target<0)target=0;p.wait();$("#canvas")[0].focus();};p.setPretarget=function(value)
{pretarget=value;p.wait();};p.setColor=function(value){color=value;};p.setWeight=function(value){weight=value;};p.setAlpha=function(value){alpha=value/100;};p.setForeground=function(value){foreground^=1;return foreground;};p.getColor=function(){return color;};p.getWeight=function(){return weight;};p.getAlpha=function(){return alpha*100;};p.getForeground=function(){return foreground;};p.toggleTree=function(target){layers[target].toggleTree();p.wait();};p.toggleDisplay=function(target){layers[target].toggleDisplay();p.wait();};p.getLayer=function(){return layer;};p.getLayers=function(){return layers;};p.BACKGROUND=function(c)
{p.background(getRGB(c).r,getRGB(c).g,getRGB(c).b);};p.RECT=function(xy,w,h,s,c,a)
{p.stroke(getRGB(c).r,getRGB(c).g,getRGB(c).b);p.fill(getRGB(c).r,getRGB(c).g,getRGB(c).b);p.rect(cardinal[0]+xy[0]+center[0],cardinal[1]+xy[1]+center[1],w,h);};p.LINE=function(xy1,xy2,c,w,a)
{p.stroke(p.color(getRGB(c).r,getRGB(c).g,getRGB(c).b,a));p.strokeWeight(w);p.line(cardinal[0]+xy1[0]+center[0],cardinal[1]+xy1[1]+center[1],cardinal[0]+xy2[0]+center[0],cardinal[1]+xy2[1]+center[1])};p.DOT=function(xy,c,w,a)
{p.stroke(p.color(getRGB(c).r,getRGB(c).g,getRGB(c).b,a));p.strokeWeight(w);p.line(cardinal[0]+xy[0]+center[0],cardinal[1]+xy[1]+center[1],cardinal[0]+xy[0]+center[0],cardinal[1]+xy[1]+center[1])};p.CIRCLE=function(xy,c,w,f,a)
{p.stroke(p.color(getRGB(c).r,getRGB(c).g,getRGB(c).b,a));p.fill(p.color(getRGB(f).r,getRGB(f).g,getRGB(f).b,a));p.ellipse(cardinal[0]+xy[0]+center[0],cardinal[1]+xy[1]+center[1],w,w)};p.IMAGE=function(url)
{img.src=url;ctx.save();ctx.globalAlpha=p.imgAlpha;ctx.drawImage(img,p.imgX+cardinal[0],p.imgY+cardinal[1]);ctx.restore();}
p.format=function()
{p.BACKGROUND(backgroundColor);var formatColor=(parseInt(getRGB(backgroundColor).r)+parseInt(getRGB(backgroundColor).g)+parseInt(getRGB(backgroundColor).b))/3>127?"#000000":"#ffffff";p.LINE([0,0],[800,0],formatColor,1,64);p.LINE([0,22],[800,22],formatColor,1,64);p.LINE([0,400],[800,400],formatColor,1,64);p.LINE([0,600],[800,600],formatColor,1,64);p.LINE([0,0],[0,600],formatColor,1,64);p.LINE([800,0],[800,600],formatColor,1,64);};p.elements=function(_foreground)
...
{p.DOT([(el.x1+el.x2)/2,(el.y1+el.y2)/2],"#ffffff",12,255);p.DOT([(el.x1+el.x2)/2,(el.y1+el.y2)/2],"#6a7495",10,255);p.DOT([(el.x1+el.x2)/2,(el.y1+el.y2)/2],"#ffffff",6,255);}}}}};p.updateEls=function()
{els=layers[layer.target].elements;}
p.addLayer=function()
{els=layers[layer.addLayer()].elements;target=-1;p.wait();}
p.delLayer=function(target)
{els=layers[layer.delLayer(target)].elements;target=-1;p.wait();}
p.layerOrderUp=function(target)
{layer.orderUp(target);target=-1;p.wait();}
p.layerOrderDown=function(target)
{layer.orderDown(target);target=-1;p.wait();}
p.layerSetLink=function(target,linkGround)
{layers[target].setLink(linkGround);p.wait();}
p.loadFirst=function()
{p.delLayer(0);}
p.loadLayer=function(name,link)
{p.addLayer();layers[layer.target].nameChange(name);layers[layer.target].setLink(link);}
p.loadLine=function(xy1,xy2,color,weight,alpha,foreground)
{layers[layer.target].addLine([xy1[0],xy1[1]],[xy2[0],xy2[1]],color,weight,alpha,foreground);p.updateEls();target=els.length-1;}
...
{p.background(getRGB(c).r,getRGB(c).g,getRGB(c).b);};p.DOT=function(w,c,a)
{p.stroke(p.color(getRGB(c).r,getRGB(c).g,getRGB(c).b,a));p.strokeWeight(w);p.line(90,90,90,90)};}
...
{processing.setColor(value);processing2.call(null,value,null);}
function weight(value)
{processing.setWeight(value);processing2.call(value,null,null);}
function alpha(value)
{processing.setAlpha(value);processing2.call(null,null,value);}
function foreground(but)
{processing.wait();but.className=processing.setForeground()?"on":"off";}
...
{processing.toggleTree(target);$(onoff).toggleClass("off");}
function toggleDisplay(onoff,target)
{processing.toggleDisplay(target);$(onoff).toggleClass("off");}
...
{if(!layers[i].display)continue;res+="<VDE LAYER=\""+layers[i].name+"\" LINK=\""+layers[i].link+"\" />";for(var j=0;j<layers[i].elements.length;j++)
...
function load()
{var layer=processing.getLayer();var layers=processing.getLayers();reset();var xml=$("#xmlLoad").val();var p1;var p2;var c;var link;var first=0;$(xml).find("JD, VDE").each(function(index,e)
{if(e.tagName=="VDE")
{if($(this).attr("LAYER")!=null&&$(this).attr("LINK")!=null)
{link=parseInt($(this).attr("LINK"));if(isNaN(link))return;if(first==0)first=1;processing.loadLayer($(this).attr("LAYER"),link);}}
if(e.tagName=="JD")
