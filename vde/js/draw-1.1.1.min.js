...
{if(p.mouseButton!=p.LEFT)return;wait=-1;setSavedMouse([getZoomedOutX(getMouseX()-getTfmX()),getZoomedOutY(getMouseY()-getTfmY())]);if(mode=="new")p.setNewline(getSavedMouseX(),getSavedMouseY(),getSavedMouseX(),getSavedMouseY());if(mode=="move1"||mode=="move2"||mode=="move3")p.setTarget(pretarget);};p.mouseReleased=function()
...
{mode="moving1";setSavedMouse([els[pretarget].x2,els[pretarget].y2]);p.setNewline(getSavedMouseX(),getSavedMouseY(),getSavedMouseX(),getSavedMouseY());p.setTarget(pretarget);p.updateLine(pretarget).display(0);}
if(mode=="move3")
{mode="moving3";setSavedMouse([els[pretarget].x1,els[pretarget].y1]);p.setNewline(getSavedMouseX(),getSavedMouseY(),getSavedMouseX(),getSavedMouseY());p.setTarget(pretarget);p.updateLine(pretarget).display(0);}
if(mode=="move2")
{mode="moving2";setSavedMouse([(els[pretarget].x1+els[pretarget].x2)/2,(els[pretarget].y1+els[pretarget].y2)/2]);oldline=[els[pretarget].x2,els[pretarget].y2,els[pretarget].x1,els[pretarget].y1];p.setTarget(pretarget);p.updateLine(pretarget).display(0);}
if(mode=="moving1")
{p.setNewline(getSavedMouseX(),getSavedMouseY(),getZoomedOutX(getMouseX()-getTfmX()),getZoomedOutY(getMouseY()-getTfmY()));p.changeSelected().moveP1([newline[2],newline[3]]);}
if(mode=="moving2")
{var moved=[getZoomedOutX(getMouseX()-getTfmX())-getSavedMouseX(),getZoomedOutY(getMouseY()-getTfmY())-getSavedMouseY()];p.setNewline(oldline[0]+moved[0],oldline[1]+moved[1],oldline[2]+moved[0],oldline[3]+moved[1]);p.changeSelected().moveP1([newline[2],newline[3]]);p.changeSelected().moveP2([newline[0],newline[1]]);}
if(mode=="moving3")
{p.setNewline(getZoomedOutX(getMouseX()-getTfmX()),getZoomedOutY(getMouseY()-getTfmY()),getSavedMouseX(),getSavedMouseY());p.changeSelected().moveP2([newline[0],newline[1]]);}};p.mouseMoved=function()
...
{el=els[i];if(helper.middle&&distance(mouse2,[(el.x1+el.x2)/2,(el.y1+el.y2)/2])<7/getZoom())
...
{var clr=getRGB(c);if(clr==null)return;p.stroke(p.color(clr.r,clr.g,clr.b,a));if(ignoreZoom==undefined)w*=zoom;xy1[0]=getZoomedXY(xy1).x;xy1[1]=getZoomedXY(xy1).y;xy2[0]=getZoomedXY(xy2).x;xy2[1]=getZoomedXY(xy2).y;p.strokeWeight(w);p.line(xy1[0]+getTfmX(),xy1[1]+getTfmY(),xy2[0]+getTfmX(),xy2[1]+getTfmY())};p.CROSS=function(xy,l,c,w,a){var clr=getRGB(c);if(clr==null)return;xy[0]=getZoomedXY(xy).x;xy[1]=getZoomedXY(xy).y;p.stroke(p.color(getRGB("#6a7495").r,getRGB("#6a7495").g,getRGB("#6a7495").b));p.strokeWeight(w*2);p.line(xy[0]+getTfmX()-l,xy[1]+getTfmY(),xy[0]+getTfmX()+l,xy[1]+getTfmY())
p.line(xy[0]+getTfmX(),xy[1]+getTfmY()-l,xy[0]+getTfmX(),xy[1]+getTfmY()+l)
clr=getRGB(c);if(clr==null)return;p.stroke(p.color(clr.r,clr.g,clr.b,a));p.strokeWeight(w);p.line(xy[0]+getTfmX()-l,xy[1]+getTfmY(),xy[0]+getTfmX()+l,xy[1]+getTfmY())
p.line(xy[0]+getTfmX(),xy[1]+getTfmY()-l,xy[0]+getTfmX(),xy[1]+getTfmY()+l)};p.IMAGE=function(url)
